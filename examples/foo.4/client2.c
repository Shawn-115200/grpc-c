...
void call_sigle_stream( grpc_c_client_t *client ) {
    ...
    for ( i = 0 ; i < 1; i++ )
    {
        ...
        ret = foo__greeter__sigle_stream__sync(client, NULL, 0, &empty_req, &r, &status, -1);
        if ( ret )
        {
        ...
        if (r) {
            ...
            if (r->user_info) {
                printf("Server User: %s, Score: %.2f\n", r->user_info->username, r->user_info->score);
            }
...
void call_sigle_stream2( grpc_c_client_t *client ) {
    ...
    for ( i = 0 ; i < 1; i++ )
    {
        ...
        printf("\tThis will invoke a blocking RPC\n");
        ret = foo__greeter__sigle_stream2__sync(client, NULL, 0, &h, &empty_rsp, &status, -1);
        if ( ret )
        {
            printf("call failed! %d\n", ret);
            if (h.user_info) {
...
int foo_client() 
{
     ...
     */
    while (1) {
        grpc_c_client_t *client = grpc_c_client_init("127.0.0.1:4000", NULL, NULL);
        if (client != NULL) {
            // 客户端初始化成功，可以继续执行
